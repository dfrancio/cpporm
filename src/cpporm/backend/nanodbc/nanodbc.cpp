...
// https://social.msdn.microsoft.com/Forums/en-US/8f40dcd8-c67f-4eba-9134-a19b9178e481
    ...
    auto s
        = std::wstring_convert<NANODBC_CODECVT_TYPE<unsigned short>, unsigned short>().from_bytes(
            in);
...
inline std::string recent_error(
    SQLHANDLE handle, SQLSMALLINT handle_type, long &native, std::string &state)
{
    ...
    {
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT)i, sql_state,
            &native_error, 0, 0, &total_bytes);
        ...
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetDiagRec), rc, handle_type, handle, (SQLSMALLINT)i, sql_state,
            &native_error, sql_message.data(), (SQLSMALLINT)sql_message.size(), &total_bytes);
...
type_incompatible_error::type_incompatible_error() : std::runtime_error("type incompatible")
{
}
...
null_access_error::null_access_error() : std::runtime_error("null access")
{
}
...
index_range_error::index_range_error() : std::runtime_error("index out of range")
{
}
...
programming_error::programming_error(const std::string &info) : std::runtime_error(info.c_str())
{
}
...
inline void allocate_environment_handle(SQLHENV &env)
{
    ...
    {
        NANODBC_CALL_RC(
            SQLSetEnvAttr, rc, env, SQL_ATTR_ODBC_VERSION, (SQLPOINTER)NANODBC_ODBC_VERSION,
            SQL_IS_UINTEGER);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(env, SQL_HANDLE_ENV);
    }
...
public:
    ...
    connection_impl() : env_(0), conn_(0), connected_(false), transactions_(0), rollback_(false)
    {
        allocate_handle(env_, conn_);
    }

    connection_impl(
        const string_type &dsn, const string_type &user, const string_type &pass, long timeout)
        : env_(0), conn_(0), connected_(false), transactions_(0), rollback_(false)
    {
        allocate_handle(env_, conn_);
        try
        {
    ...
    void enable_async(void *event_handle)
    {
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLSetConnectAttr, rc, conn_, SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE,
    ...
    void async_complete()
    {
        ...
        NANODBC_CALL_RC(
            SQLSetConnectAttr, rc, conn_, SQL_ATTR_ASYNC_DBC_FUNCTIONS_ENABLE,
            (SQLPOINTER)SQL_ASYNC_DBC_ENABLE_OFF, SQL_IS_INTEGER);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
    }
        ...
        void *event_handle = nullptr)
    {
        ...
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLConnect), rc, conn_, (NANODBC_SQLCHAR *)dsn.c_str(), SQL_NTS,
    ...
    connect(const string_type &connection_string, long timeout, void *event_handle = nullptr)
    {
        ...
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLDriverConnect), rc, conn_, 0,
            (NANODBC_SQLCHAR *)connection_string.c_str(), SQL_NTS, nullptr, 0, nullptr,
            SQL_DRIVER_NOPROMPT);
        if (!success(rc) && (event_handle == nullptr || rc != SQL_STILL_EXECUTING))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
    ...
    string_type dbms_name() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetInfo), rc, conn_, SQL_DBMS_NAME, name,
            sizeof(name) / sizeof(NANODBC_SQLCHAR), &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
    ...
    string_type dbms_version() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetInfo), rc, conn_, SQL_DBMS_VER, version,
            sizeof(version) / sizeof(NANODBC_SQLCHAR), &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
    ...
    string_type driver_name() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetInfo), rc, conn_, SQL_DRIVER_NAME, name,
            sizeof(name) / sizeof(NANODBC_SQLCHAR), &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
        return string_type(&name[0], &name[strarrlen(name)]);
    ...
    string_type database_name() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetInfo), rc, conn_, SQL_DATABASE_NAME, name,
            sizeof(name) / sizeof(NANODBC_SQLCHAR), &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
        return string_type(&name[0], &name[strarrlen(name)]);
    ...
    string_type catalog_name() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLGetConnectAttr), rc, conn_, SQL_ATTR_CURRENT_CATALOG, name,
            sizeof(name) / sizeof(NANODBC_SQLCHAR), &length);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(conn_, SQL_HANDLE_DBC);
        return string_type(&name[0], &name[strarrlen(name)]);
...
public:
    ...
    transaction_impl(const class connection &conn) : conn_(conn), committed_(false)
    {
        if (conn_.transactions() == 0 && conn_.connected())
        {
            RETCODE rc;
            NANODBC_CALL_RC(
                SQLSetConnectAttr, rc, conn_.native_dbc_handle(), SQL_ATTR_AUTOCOMMIT,
    ...
    ~transaction_impl() NANODBC_NOEXCEPT
    {
        ...
        if (conn_.transactions() == 0 && conn_.connected())
        {
            ...
            NANODBC_CALL(
                SQLSetConnectAttr, conn_.native_dbc_handle(), SQL_ATTR_AUTOCOMMIT,
...
public:
    ...
    RETCODE prepare(const string_type &query, long timeout, void *event_handle = nullptr)
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLPrepare), rc, stmt_, (NANODBC_SQLCHAR *)query.c_str(),
    ...
    void timeout(long timeout)
    {
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLSetStmtAttr, rc, stmt_, SQL_ATTR_QUERY_TIMEOUT, (SQLPOINTER)(std::intptr_t)timeout,
            0);
    ...
    void enable_async(void *event_handle)
    {
        ...
        {
            NANODBC_CALL_RC(
                SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE, (SQLPOINTER)SQL_ASYNC_ENABLE_ON,
                SQL_IS_INTEGER);
            if (!success(rc))
                NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
            async_enabled_ = true;
    ...
    void disable_async() const
    {
        if (async_enabled_)
        {
            RETCODE rc;
            NANODBC_CALL_RC(
                SQLSetStmtAttr, rc, stmt_, SQL_ATTR_ASYNC_ENABLE, (SQLPOINTER)SQL_ASYNC_ENABLE_OFF,
                SQL_IS_INTEGER);
            if (!success(rc))
                NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
            async_enabled_ = false;
    ...
    bool async_execute_direct(
        class connection &conn, void *event_handle, const string_type &query, long batch_operations,
    ...
    bool async_execute(
        void *event_handle, long batch_operations, long timeout, statement &statement)
    {
    ...
    result execute_direct(
        class connection &conn, const string_type &query, long batch_operations, long timeout,
        statement &statement)
    {
        ...
        void *event_handle = nullptr)
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLSetStmtAttr, rc, stmt_, SQL_ATTR_PARAMSET_SIZE,
            (SQLPOINTER)(std::intptr_t)batch_operations, 0);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
    ...
    RETCODE just_execute(
        long batch_operations, long timeout, statement & /*statement*/,
        void *event_handle = nullptr)
    {
        RETCODE rc;
    ...
    result procedure_columns(
        const string_type &catalog, const string_type &schema, const string_type &procedure,
        const string_type &column, statement &statement)
    {
        if (!open())
            throw programming_error("statement has no associated open connection");
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLProcedureColumns), rc, stmt_,
            ...
            (schema.empty() ? 0 : SQL_NTS), (NANODBC_SQLCHAR *)procedure.c_str(), SQL_NTS,
    ...
    void prepare_bind(
        short param, std::size_t elements, param_direction direction, SQLSMALLINT &data_type,
        SQLSMALLINT &param_type, SQLULEN &parameter_size, SQLSMALLINT &scale)
    {
        ...
        NANODBC_CALL_RC(
            SQLDescribeParam, rc, stmt_, param + 1, &data_type, &parameter_size, &scale, &nullable);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(stmt_, SQL_HANDLE_STMT);
    ...
    void bind_null(short param, std::size_t elements)
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLBindParameter, rc, stmt_, param + 1, param_type, SQL_C_CHAR, data_type,
            parameter_size, // column size ignored for many types, but needed for strings
            0,
...
void statement::statement_impl::bind_strings(
    ...
    const bool *nulls, const string_type::value_type *null_sentry, param_direction direction)
{
    SQLSMALLINT data_type;
    SQLSMALLINT param_type;
    SQLULEN parameter_size;
...
public:
...
#endif
    {
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLSetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_ARRAY_SIZE,
            (SQLPOINTER)(std::intptr_t)rowset_size_, 0);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);

        NANODBC_CALL_RC(
            SQLSetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROWS_FETCHED_PTR,
            &row_count_, 0);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
    ...
    unsigned long position() const
    {
        ...
        RETCODE rc;
        NANODBC_CALL_RC(
            SQLGetStmtAttr, rc, stmt_.native_statement_handle(), SQL_ATTR_ROW_NUMBER, &pos,
            SQL_IS_UINTEGER, 0);
        if (!success(rc))
            NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
...
private:
    ...
    void auto_bind()
    {
        ...
        {
            NANODBC_CALL_RC(
                NANODBC_FUNC(SQLDescribeCol), rc, stmt_.native_statement_handle(), i + 1,
                (NANODBC_SQLCHAR *)column_name, sizeof(column_name) / sizeof(NANODBC_SQLCHAR), &len,
                &sqltype, &sqlsize, &scale, &nullable);
            if (!success(rc))
                NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
        ...
        for (SQLSMALLINT i = 0; i < n_columns; ++i)
        {
            ...
            {
                NANODBC_CALL_RC(
                    SQLBindCol, rc, stmt_.native_statement_handle(), i + 1, col.ctype_, 0, 0,
                    col.cbdata_);
                if (!success(rc))
                    NANODBC_THROW_DATABASE_ERROR(stmt_.native_statement_handle(), SQL_HANDLE_STMT);
            }
            else
            {
                ...
                NANODBC_CALL_RC(
                    SQLBindCol, rc, stmt_.native_statement_handle(),
                    i + 1, // ColumnNumber
...
inline void result::result_impl::get_ref_impl<string_type>(short column, string_type &result) const
{
    ...
    case SQL_C_LONG:
    {
        ...
        const int32_t data
            = *reinterpret_cast<int32_t *>(col.pdata_ + rowset_position_ * col.clen_);
        const int bytes
    ...
    case SQL_C_SBIGINT:
    {
        ...
        std::string buffer;
        column_size = numeric_limits<intmax_t>::digits + 1;
        buffer.reserve(column_size + 1); // ensure terminating null
        buffer.resize(buffer.capacity());
        using std::fill;
        fill(buffer.begin(), buffer.end(), '\0');
        const intmax_t data
            = *reinterpret_cast<intmax_t *>(col.pdata_ + rowset_position_ * col.clen_);
        const int bytes
    ...
    case SQL_C_DOUBLE:
    {
        ...
        const int bytes = std::snprintf(
            const_cast<char *>(buffer.data()), width + 1,
...
std::list<driver> list_drivers()
{
    ...
    do
    {
        NANODBC_ASSERT(env);
        NANODBC_CALL_RC(
            NANODBC_FUNC(SQLDrivers), rc, env,
...
connection::connection(const connection &rhs) : impl_(rhs.impl_)
{
}
...
bool connection::async_connect(
    const string_type &dsn, const string_type &user, const string_type &pass, void *event_handle,
    long timeout)
{
...
bool connection::async_connect(
    const string_type &connection_string, void *event_handle, long timeout)
{
...
transaction::transaction(const class connection &conn) : impl_(new transaction_impl(conn))
{
}

transaction::transaction(const transaction &rhs) : impl_(rhs.impl_)
{
}
...
statement::statement(class connection &conn) : impl_(new statement_impl(conn))
{
}
...
catalog::tables::tables(result &find_result) : result_(find_result)
{
}
...
catalog::table_privileges::table_privileges(result &find_result) : result_(find_result)
{
}
...
catalog::primary_keys::primary_keys(result &find_result) : result_(find_result)
{
}
...
catalog::columns::columns(result &find_result) : result_(find_result)
{
}
...
catalog::catalog(connection &conn) : conn_(conn)
{
}

catalog::tables catalog::find_tables(
    const string_type &table, const string_type &type, const string_type &schema,
    const string_type &catalog)
{
    ...
    RETCODE rc;
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)(catalog.empty() ? nullptr : catalog.c_str()),
        (catalog.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(table.empty() ? nullptr : table.c_str()), (table.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(type.empty() ? nullptr : type.c_str()), (type.empty() ? 0 : SQL_NTS));
    if (!success(rc))
...
catalog::table_privileges catalog::find_table_privileges(
    const string_type &catalog, const string_type &table, const string_type &schema)
{
    // Passing a null pointer to a search pattern argument does not
    // constrain the search for that argument; that is, a null pointer and
    // the search pattern % (any characters) are equivalent.
    ...
    RETCODE rc;
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLTablePrivileges), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)(catalog.empty() ? nullptr : catalog.c_str()),
        (catalog.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
...
catalog::columns catalog::find_columns(
    const string_type &column, const string_type &table, const string_type &schema,
    const string_type &catalog)
{
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLColumns), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)(catalog.empty() ? nullptr : catalog.c_str()),
        (catalog.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
...
catalog::primary_keys catalog::find_primary_keys(
    const string_type &table, const string_type &schema, const string_type &catalog)
{
    statement stmt(conn_);
    RETCODE rc;
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLPrimaryKeys), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)(catalog.empty() ? nullptr : catalog.c_str()),
        (catalog.empty() ? 0 : SQL_NTS),
        (NANODBC_SQLCHAR *)(schema.empty() ? nullptr : schema.c_str()),
        (schema.empty() ? 0 : SQL_NTS),
...
std::list<string_type> catalog::list_catalogs()
{
    ...
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)SQL_ALL_CATALOGS, 1, (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0,
        (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0);
    if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
...
std::list<string_type> catalog::list_schemas()
{
    ...
    NANODBC_CALL_RC(
        NANODBC_FUNC(SQLTables), rc, stmt.native_statement_handle(),
        (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR *)SQL_ALL_SCHEMAS, 1,
        (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0, (NANODBC_SQLCHAR *)NANODBC_TEXT(""), 0);
    if (!success(rc))
        NANODBC_THROW_DATABASE_ERROR(stmt.native_statement_handle(), SQL_HANDLE_STMT);
